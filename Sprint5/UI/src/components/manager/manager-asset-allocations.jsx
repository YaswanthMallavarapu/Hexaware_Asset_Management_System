...
const ManagerAssetAllocations = () => {
  ...
  const fetchAllocations = async (currentPage, statFilter) => {
    setError('');
    try {
      let res;
      if (statFilter !== 'ALL') {
        res = await axios.get(
          `${BASE}/get/status/${statFilter}?page=${currentPage}&size=${size}`,
          config
        );
      } else {
        res = await axios.get(
  ...
  const handleAcceptReturn = async (id) => {
    ...
    setError('');
    try {
      await axios.put(`${BASE}/accept-return-request/${id}`, {}, config);
      fetchAllocations(page, statusFilter);
    } catch (err) {
      handleError(err);
    } finally {
  ...
  const allocationStatusStyle = (status) => {
    switch (status) {
      case 'ALLOCATED':
        return { bg: '#f0fdf4', color: '#16a34a', label: '● Allocated' };
      case 'RETURN_REQUESTED':
        return { bg: '#fffbeb', color: '#d97706', label: '↩ Return Requested' };
      case 'RETURNED':
        return { bg: '#f8fafc', color: '#64748b', label: '✓ Returned' };
      default:
      ...
      <div className="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted small mb-0">
            View allocations and process return requests
          </p>
        </div>
      </div>
            ...
            <tbody>
              {allocations.length === 0 ? (
                ...
                allocations.map((alloc, index) => {
                  const sStyle = allocationStatusStyle(alloc.status);
                  const isActioning = actioningId === alloc.id;
                  return (
                    <tr
                      key={alloc.id}
                    ...
                    >
                      ...
                      >
                        {alloc.status === 'RETURN_REQUESTED' ? (
                          <button
                            onClick={() => handleAcceptReturn(alloc.id)}
                          ...
                          >
                            ...
                            {isActioning ? 'Processing...' : 'Accept Return'}
                          </button>
                        ) : (
                          <span
                            className="text-muted"
...
export default ManagerAssetAllocations;