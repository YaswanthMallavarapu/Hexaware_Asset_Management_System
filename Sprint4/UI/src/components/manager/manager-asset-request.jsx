...
const ManagerAssetRequests = () => {
  ...
  const MANAGER_BASE = 'http://localhost:8082/api/manager';
  ...
  const handleApprove = async (id) => {
    ...
    setError('');
    try {
      await axios.put(`${MANAGER_BASE}/asset-request/approve/${id}`, {}, config);
      fetchRequests(page, statusFilter);
    } catch (err) {
      handleError(err);
    } finally {
  ...
  const handleReject = async (id) => {
    ...
    setError('');
    try {
      await axios.put(`${MANAGER_BASE}/asset-request/reject/${id}`, {}, config);
      fetchRequests(page, statusFilter);
    } catch (err) {
      handleError(err);
    } finally {
  ...
  const requestStatusStyle = (status) => {
    switch (status) {
      case 'APPROVED':
        return { bg: '#f0fdf4', color: '#16a34a', label: '✓ Approved' };
      case 'PENDING':
            ...
            <tbody>
              {requests.length === 0 ? (
                ...
                requests.map((req, index) => {
                  const sStyle = requestStatusStyle(req.status);
...
export default ManagerAssetRequests;