...
const ManagerAudit = () => {
  ...
  const AUDIT_BASE = 'http://localhost:8080/api/asset-audit';
  ...
  const fetchAudits = async (pg) => {
    ...
    try {
      const res = await axios.get(
        `${AUDIT_BASE}/get-all?page=${pg}&size=${auditSize}`,
        config
      );
  ...
  // ── Pagination footer (reused for both views) ────────────────────────────
  const PaginationFooter = ({ page, hasMore, onPrev, onNext }) => (
    <div
        ...
        <>
          ...
          >
                          ...
                          {audit.manager ? (
                            <>
                              <div className="fw-semibold" style={{ fontSize: '0.9rem', color: '#1e293b' }}>
                                {audit.manager.firstName ?? ''} {audit.manager.lastName ?? audit.manager.username ?? ''}
                              </div>
            ...
            <PaginationFooter
              ...
              onPrev={() => { if (auditPage > 0) setAuditPage((p) => p - 1); }}
              onNext={() => { if (auditHasMore) setAuditPage((p) => p + 1); }}
            />
          </div>
        </>
      )}
        ...
        <>
          ...
          <div className="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-3">
            <div>
              ...
              <p className="text-muted small mb-0">
                ...
                {selectedAudit.manager && (
                  <> &middot; {selectedAudit.manager.firstName ?? ''} {selectedAudit.manager.lastName ?? selectedAudit.manager.username ?? ''}</>
                )}
              </p>
            </div>
          </div>
          ...
          >
            ...
            <PaginationFooter
              ...
              onPrev={() => { if (resultsPage > 0) setResultsPage((p) => p - 1); }}
              onNext={() => { if (resultsHasMore) setResultsPage((p) => p + 1); }}
            />
          </div>
        </>
      )}
